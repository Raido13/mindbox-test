...
export const Todos = ({ todosData, toggleTodoStatus }: TodosProps) => {
  const todosList = useMemo(
    () =>
      todosData.map((t) => (
        <Todo todo={t} toggleTodoStatus={toggleTodoStatus} key={t.id} />
      )),
    [todosData, toggleTodoStatus]
  );

  return <ul className={s.todos}>{todosList}</ul>;
};
  ...
  return (
    <li className={s.todo}>
      <label
        className={cn(
          s.todo__task,
          todo.status === TodoStatus.Completed && s.todo__task__completed
        )}
      >
        <span
          className={cn(
            s.todo__status,
            todo.status === TodoStatus.Active && s.todo__status__active
          )}
        >
